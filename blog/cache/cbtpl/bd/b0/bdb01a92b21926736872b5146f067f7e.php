<article id="p<?php echo context::global_filters($_ctx->posts->post_id,array (
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryID'); ?>" class="post short <?php if (($_ctx->posts->index()+1)%2 == 1) { echo 'odd'; } ?> <?php if ($_ctx->posts->index() == 0) { echo 'first'; } ?>" lang="<?php if ($_ctx->posts->post_lang) { echo context::global_filters($_ctx->posts->post_lang,array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => 0,
  'cut_string' => 0,
  ...
  'strip_tags' => 0,
  'iso8601' => '1',
),'EntryDate'); ?>"><i class="far fa-calendar-alt"></i> <?php echo context::global_filters($_ctx->posts->getDate('',''),array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => 0,
  'cut_string' => 0,
  ...
  'strip_tags' => 0,
	...
	<footer class="post-meta">
		<p class="post-info">
			<span class="post-author"><i class="far fa-user"></i> <?php echo context::global_filters($_ctx->posts->getAuthorLink(),array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => 0,
  'cut_string' => 0,
...
while ($_ctx->categories->fetch()) : ?><a
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryCategoryURL'); ?>"><i class="far fa-folder"></i> <?php echo context::global_filters($_ctx->posts->cat_title,array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => '1',
  'cut_string' => 0,
...
$_ctx->meta = $core->meta->getMetaRecordset($_ctx->posts->post_meta,'tag'); $_ctx->meta->sort('meta_id_lower','asc'); ?><?php while ($_ctx->meta->fetch()) : ?>
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'TagURL'); ?>"><i class="fas fa-tag"></i> <?php echo context::global_filters($_ctx->meta->meta_id,array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => 0,
  'cut_string' => 0,
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryURL'); ?>#comments" class="comment_count"><i class="far fa-comments"></i> <?php if ($_ctx->posts->nb_comment == 0) {
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryURL'); ?>#pings" class="ping_count"><i class="far fa-comment-dots"></i> <?php if ($_ctx->posts->nb_trackback == 0) {
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryURL'); ?>#attachments" class="attach_count"><i class="fas fa-paperclip"></i> <?php if ($_ctx->posts->countMedia('attachment') == 0) {
